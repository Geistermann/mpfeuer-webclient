...
<div class="container mx-auto p-4">
    ...
    {{-- Ergebnis-Karten --}}
    <h1 class="text-2xl font-bold mb-4">Suchergebnisse für <span class="font-mono bg-gray-100 px-2 py-1 rounded">{{ Str::upper($barcode) }}</span></h1>
                            ...
                            <tbody>
                                @forelse($pruefungen as $p)
                                    ...
                                    <tr class="{{ $isDone ? 'bg-green-50' : ($isFuture ? 'bg-yellow-50' : 'bg-red-50') }}">
                                        <td class="p-2 border">{{ $p->$colDat }}</td>
                                        <td class="p-2 border text-center">{{ $p->$colLang }}</td>
                                        <td class="p-2 border text-center">{{ $p->$colHdz ?? '—' }}</td>
                    ...
                    @else
                        <div id="newpruefung-{{ $loop->index }}" class="block">
                            <form action="{{ route('pruefung.store') }}" method="POST" class="bg-gray-50 p-3 rounded">
                                ...
                                <div class="mb-3">
                                    <label for="par_pruef_lang" class="block font-semibold mb-1">Prüfung auswählen:</label>
                                    <select name="par_pruef_lang" id="par_pruef_id" class="border rounded w-full p-2" required>
                                        @forelse($pruefModel::getAvailablePruefungen() as $option)
